...
- 只有 active profile 的規則、skill、設定會出現在目標專案裡；未啟用 profile 的東西實體上不存在，不是靠文件告誡 agent 不要用——理由與 walking skeleton 階段見 [ADR 0005](adr/0005-core-profile-isolation.md)。`profiles/experimental/` 目前只是決策文件，還沒有實際內容，`apply.sh --profile experimental` 會明確報錯而不是產生半殘缺的專案。
