# 研究型 Agent 的實驗生命週期：一手來源核查與本專案落地建議

更新日期：2026-08-11

## 研究問題與範圍

本文件回答一個工程問題：本專案目前以 `definitions / runs / comparisons / claims / audits / gates / metrics / artifacts` 組成的 Experiment canonical model，如何承接研究型 agent 從題目形成、反證搜尋、實驗定義、執行、比較、主張、稽核到失敗更新的完整流程；提示詞應在哪些節點增強流程，又有哪些判定必須由 deterministic code 負責。

只採正式論文、官方技術報告、標準、官方文件與官方 source repo。每節刻意拆成「來源直接支持」與「對本專案的推論」，避免把外部系統的做法冒充成已被證明適合本專案的結論。

## 結論先行

### VERIFIED：外部證據共同支持的最小骨架

1. **研究題目形成必須是一個可稽核產物，不只是 prompt 對話。** FirstResearch 的 Research Question Certificate 明確串起 primitives、assumptions、mechanism、tension、falsifiable hypothesis、minimal decisive test、expected observations 與 failure update；其初步 ablation 也顯示 certificate-centered core 是目前最有證據的部分，但作者明確警告樣本小、仍依賴 LLM judges，不能外推成普遍科學能力。[FirstResearch 論文](https://arxiv.org/pdf/2607.05682)
2. **生成與驗證要分離，而且驗證必須能拒答。** Aletheia 以 Generator、Verifier、Reviser 迴圈工作，達到 verifier 接受或預設嘗試上限才停止；論文特別指出 verifier 分離與「承認沒有解」可提高 answered subset 的可靠度。[Aletheia 論文](https://arxiv.org/pdf/2602.10177)
3. **有廉價、客觀 evaluator 的問題，應讓 code execution 與 evaluator 主導選擇。** FunSearch 以 `evaluate` function、執行限制、分數與正確性過濾候選；AlphaEvolve 也明確把人類負責的「What」與自動搜尋的「How」分開，並以一個或多個 evaluator 持續回饋。[FunSearch 論文](https://www.nature.com/articles/s41586-023-06924-6)、[AlphaEvolve 技術報告](https://storage.googleapis.com/deepmind-media/DeepMind.com/Blog/alphaevolve-a-gemini-powered-coding-agent-for-designing-advanced-algorithms/AlphaEvolve.pdf)
4. **研究執行應分階段與逐級放大 compute。** AlphaEvolve 提供由便宜到昂貴的 evaluation cascade；AI Scientist-v2 將實驗分成 feasibility、hyperparameter tuning、research agenda、ablation，並在階段結束做 replication 與 checkpoint。[AlphaEvolve 技術報告](https://storage.googleapis.com/deepmind-media/DeepMind.com/Blog/alphaevolve-a-gemini-powered-coding-agent-for-designing-advanced-algorithms/AlphaEvolve.pdf)、[AI Scientist-v2 論文](https://arxiv.org/pdf/2504.08066)
5. **novelty 與 claim correctness 不能靠模型記憶。** AI co-scientist 的 search ablation 顯示，無外部搜尋時會明顯高估 novelty；Aletheia 也觀察到網路工具能把虛構 citation 降低，但仍可能誤引真實論文的內容。因此「有來源」仍不等於「來源支持該主張」。[AI co-scientist 論文](https://arxiv.org/pdf/2502.18864)、[Aletheia 論文](https://arxiv.org/pdf/2602.10177)
6. **完整 provenance 應保存實體、活動、責任者及 derivation 關係。** W3C PROV 以 Entity、Activity、Agent 與 `used / wasGeneratedBy / wasDerivedFrom / wasAssociatedWith` 表達來源鏈；MLflow 的官方 tracking model 則驗證了實驗系統最少要能關聯 run、parameters、code version、metrics、dataset 與 artifacts。[W3C PROV-O](https://www.w3.org/TR/prov-o/)、[MLflow Tracking 官方文件](https://mlflow.org/docs/latest/ml/tracking/)

### 對本專案的建議（INFERENCE）

目前 canonical model 的中後段骨架是對的，不需要另造一套 autonomous-research framework。最缺的是前端的「題目如何被推導」與橫跨全程的「誰、用哪個 prompt／模型／工具、從哪些 evidence 產生什麼」：

```text
Topic intake
  → Research Question Certificate
  → preflight gate（可行性／novelty／反例／倫理）
  → locked experiment definition
  → pilot run
  → compute gate
  → full / replication / ablation runs
  → deterministic comparison + confound audit
  → candidate claim
  → claim-evidence audit
  → independent review
  → accept / reject / revise / inconclusive
  → failure update 回到 certificate 或 definition
```
...
- Prompt repair 只提出修訂版 certificate；是否通過 hard gate 仍由 code 決定。
...
### 3. FunSearch 與 AlphaEvolve：prompt 負責候選，evaluator 負責現實
...
### 4. AI co-scientist：反例搜尋與 novelty review 有用，但 ranking 不是事實
...
### 5. AI Scientist-v2：分階段與 replication 值得借，無限制 tree search 不值得先借
...
| 能力 | Prompt／agent 可負責 | Deterministic code 必須負責 | 禁止做法 |
|---|---|---|---|
...
7. **建立完整 RAG walking-skeleton fixture。** 驗收：topic/certificate → locked definition → pilot → gate → 3 replication runs → comparison → claim → audit → review 全鏈可由 fixtures 重放；至少含 confounded、failed、inconclusive 三條反例。

### P1：在有結構後再加 prompt 增強

8. **建立少量版本化 prompt roles，而非總控 mega-prompt。** 首批只做：`question-builder`、`counterexample-reviewer`、`experiment-design-critic`、`failure-diagnoser`、`claim-writer`、`evidence-reviewer`。驗收：每個 prompt 輸出 typed JSON、版本與 hash 進 provenance，且無權直接改 gate 狀態。
...
- 不做一個能同時 ideate、code、judge、promote、publish 的單一 research-agent prompt。
- 不用 LLM score 取代 metric、schema、hash、config diff、budget 或 gate。
- 不把所有 reasoning transcript 當 provenance；只保存公開、結構化、對決策有用的 derivation／interaction artifacts。
- 不把 FirstResearch 的 LLM rubric 分數當本專案 production gate。
- 不先做通用 multi-agent orchestrator、tree database、prompt optimizer。
- 不以 Viewer 補寫缺失資料；Viewer 繼續唯讀，錯誤應回到 producer／pipeline 修正。

## 預登記的改判條件

目前建議是「先補 certificate/provenance/gates/audit，再加少量 prompt roles；不建大型 autonomous coordinator」。以下證據會推翻或調整它：

1. 若現有 definition 已完整、穩定地保存 mechanism、falsifier、failure update 與 sources，則不新增重複 `derivation`，只補 lint／UI。
2. 若實際使用證明研究候選量非常少、counterexample reviewer 沒有發現人工漏項，則 prompt P1 降級，不常駐。
3. 若固定 benchmark 顯示 agentic branch search 在相同 budget 下穩定優於線性 lifecycle，且 evaluator gaming 未升高，才把 P2 tree search 提前。
4. 若同模型 reviewer 與人類 expert 的一致性在高風險 claim 上不足，改為 human-required gate，不再允許 model-only disposition。
5. 若 prompt 不是系統行為的一部分（例如純 deterministic RAG pipeline），則 prompt provenance 僅要求記錄 generator/evaluator prompts，不強迫所有 run 都有 prompt 欄位。
