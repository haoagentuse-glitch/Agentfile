...
追加於 core-superpowers 規則之後，只講實驗型專案（RAG、agent 架構、機器學習／深度學習、模擬、檢索與排序、最佳化、演算法比較）的特化規則，不改寫 core 規則。目標不是把分數做高，是維持可解釋性、可歸因性、可重現性與結論可信度——程式能跑不代表結論可信。這個 profile 限制的是「怎麼下結論」，不是「怎麼寫程式」。
